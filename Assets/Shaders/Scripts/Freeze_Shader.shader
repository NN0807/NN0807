Shader "Unlit/Freeze_Shader"
{
    // PropertiesはUnityのマテリアルの設定画面で値を指定することができるようにするもの。";"は要らない
    // 変数名("表示するラベル名", プロパティのタイプ) = デフォルト値 // この行を必要なだけ
    Properties
    {
        _MainTex              ("Texture"    ,                       2D) = "white" {}
        _Shader_Color         ("Shader Color",                   Color) = (0.5,0.5,0.5)
        _Specular_Power       ("Specular Power",       Range(  0,  30)) = 1
        _Specular_Color       ("Specular Color",                 Color) = (0.5,0.5,0.5)
    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            fixed4 frag(v2f i) : SV_Target 
            {
                ...
                // リムライト
                float Rim_Light_Rate = 1 - smoothstep(0, _RimLightBorder, dot(i.normal, i.viewDir));
                Rim_Light_Rate       =           lerp(0,  Rim_Light_Rate,               freezeRate);
                col.rgb             +=           lerp(0, _RimLightColor,            Rim_Light_Rate);
