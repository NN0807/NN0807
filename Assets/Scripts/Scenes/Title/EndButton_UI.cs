...
public class EndButton_UI : MonoBehaviour
{
    // アタッチするボタン
    ...
    // キーマウ用のボタン画像
    public Sprite _endButtonSprite;

    // コントローラー用のボタン画像
    public Sprite _endAButtonSprite;
    ...
    void Start()
    {
        ...
        if (_endButton != null)
        {
            _endButton.onClick.AddListener(OnStartButtonClick);
        }
    ...
    void Update()
    {
        ...
        {
            // ゲームパッド用の画像に差替える
            _image.sprite = _endAButtonSprite;

            // 現在選択されているGameObjectを取得
            buttonToSelect = EventSystem.current.currentSelectedGameObject;

            // スタートボタンが有り、名前が"End Button"であれば
            if (buttonToSelect != null && buttonToSelect.name == "End Button")
            {
                // シェーダーに色を渡す
        ...
        {
            // ゲームパッド用の画像に差替える
            _image.sprite = _endButtonSprite;
            ...
            if (mousePos.x >= 690.0f && mousePos.x <= 1230.0f &&
                mousePos.y >=  34.0f && mousePos.y <=  152.0f)
            {
                Debug.Log("終了ボタンがに重なっています！");
