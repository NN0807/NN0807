...
public class NetWork_CharacterCollider : MonoBehaviour, NetWork_ICharacterPart
{
    // データアセット
    public CharacterParamAsset characterParamAsset;

    // マネージャー
    private NetWork_CharacterManager _characterManager;
    ...
    public NetWork_CharacterManager _hitCharacterManager;
    ...
    public void Initialize(NetWork_CharacterManager manager)
    {
    ...
    public void UpdatePart(NetWork_CharacterManager manager)
    {
    ...
    void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("Weapon") && _hasWeaponEntered)
        {
            ...
            if (_characterManager._characterNumber != other.transform.root.gameObject.GetComponent<NetWork_CharacterManager>()._characterNumber)
            {
                // 相手が攻撃中なら
                if (other.transform.root.gameObject.GetComponent<NetWork_CharacterManager>().IsCurrentlyAttacking())
                {
        ...
        {
            // 自分が攻撃中なら
            if (this.transform.root.gameObject.GetComponent<NetWork_CharacterManager>().IsCurrentlyAttacking())
            {
    ...
    void OnTriggerEnter(Collider collision)
    {
        ...
        if (collision.gameObject.CompareTag("Weapon") && !_hasWeaponEntered)
        {
            ...
            // 攻撃を与えたCharacterの"CharacterManager"を取得する
            _hitCharacterManager = collision.transform.root.gameObject.GetComponent<NetWork_CharacterManager>();

            // 自身の体に攻撃が当たった場合に、コリジョン処理を無効化する
            if (_characterManager._characterNumber != _hitCharacterManager._characterNumber)
            {
                // 相手が攻撃中なら
                if (collision.transform.root.gameObject.GetComponent<NetWork_CharacterManager>().IsCurrentlyAttacking())
                {
                    // すでに処理済みなら無視
