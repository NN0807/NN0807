...
public class NetWork_CharacterManager : MonoBehaviourPunCallbacks
{
    // モデルスクリプト
    [SerializeField]
    public NetWork_CharacterModel     _characterModel;
    // 操作スクリプト
    [SerializeField]
    public NetWork_CharacterOperation _characterOperation;
    // 移動スクリプト
    public NetWork_CharacterMove      _characterMove;
    // UIスクリプト
    [SerializeField]
    public NetWork_CharacterUI        _characterUI;

    // 生成されたキャラクターパーツ登録用リスト
    private List<NetWork_ICharacterPart>     characterParts = new List<NetWork_ICharacterPart>();
    // コライダー登録用リスト
    private List<NetWork_CharacterCollider>  colliders      = new List<NetWork_CharacterCollider>();
    // アニメーション登録用リスト
    private List<NetWork_CharacterAnimation> animations     = new List<NetWork_CharacterAnimation>();
    ...
    {
        // 全キャラクタースクリプトを初期化
        var characterPartsArray = part.GetComponentsInChildren<NetWork_ICharacterPart>();
        ...
        var collider = part.GetComponent<NetWork_CharacterCollider>();
        if (collider != null) colliders.Add(collider);
        var animation = part.GetComponent<NetWork_CharacterAnimation>();
        if (animation != null) animations.Add(animation);
        var move = part.GetComponent<NetWork_CharacterMove>();
        if (move != null)
        {
            _characterMove = move;
            // 脚部パーツの剛体を取得
        ...
        var ui = part.GetComponent<NetWork_CharacterUI>();
